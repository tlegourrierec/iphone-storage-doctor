...
| `Le lien USB a lâché en cours d'opération` | Le plus souvent un second `ipsd` lancé en parallèle : usbmuxd ne gère pas deux sessions simultanées sur un même appareil. Attends la fin de la première. Sinon, garde le téléphone déverrouillé et branche-le directement sur le Mac, pas sur un hub. |
